...
**Производитель** реализует интерфейс [`EventProducer`](../../dispatcher/EventProducer.java) с единственным методом `publish(event)`. 
...
Для демонстрации работы создан [`CommandLineProducer`](./CommandLineProducer.java), который при запуске приложения создаёт два события `ExampleEvent`.
